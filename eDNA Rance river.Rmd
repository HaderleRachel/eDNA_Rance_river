...
stations <- data.frame(
  ...
  lon = c(-2.024, -2.023, -2.007, -1.967, -2.017),
  lat = c(48.625, 48.606, 48.595, 48.555, 48.484)
)
...
# Load Lidar shapefile (adjust path as needed)
...
lidar_map <- st_transform(lidar_map, crs = 4326)

# Define geographic bounding box for inset map (Brittany region)
brittany_bbox <- c(xmin = -3.5, xmax = -1.5, ymin = 47.5, ymax = 49.5)

# Main map with station labels
main_map <- ggplot() +
  geom_sf(data = lidar_map, fill = "grey90", color = NA) +
  geom_label_repel(data = stations %>%
                     mutate(x_label = lon + c(0.002, -0.002, 0.0015, -0.002, 0.001),
                            y_label = lat + c(0.0015, 0.001, -0.001, -0.002, -0.004)),
                   aes(x = x_label, y = y_label, label = name),
                   color = "black", fontface = "bold", size = 3) +
  geom_segment(aes(x = -2.01, y = 48.496, xend = -1.99, yend = 48.486), color = "blue", size = 0.8) +
  geom_segment(aes(x = -2.04, y = 48.614, xend = -2.001, yend = 48.624), color = "blue", size = 0.8) +
  annotate("text", x = -1.99, y = 48.484, label = "Le Châtelier", color = "blue", hjust = -0.2, vjust = 0.5, size = 3.5) +
  annotate("text", x = -2.000, y = 48.624, label = "Tidal power plant", color = "blue", hjust = -0.2, vjust = 0.5, size = 3.5) +
  annotation_scale(location = "bl", width_hint = 0.2) +
  annotation_north_arrow(location = "bl", which_north = "true",
                         pad_x = unit(0.3, "in"), pad_y = unit(0.2, "in"),
                         style = north_arrow_fancy_orienteering,
                         height = unit(1, "cm"), width = unit(1, "cm")) +
  coord_sf(xlim = c(-2.1, -1.87), ylim = c(48.475, 48.63), expand = FALSE) +
...
combined_map <- ggdraw() +
  ...
  draw_plot(overview_map, 0.05, 0.05, 0.25, 0.25)
...
ggsave("Fig2.png", plot = p, dpi = 1200, width = 10, height = 5)
...
FISH <- ggplot(numbFish, aes(x = Stat, y = freq, group = Category, colour = Category)) +
  geom_point(size = 3) + 
  geom_line(size = 1.2) +
  scale_colour_manual(values = c("orange", "brown", "green", "darkblue", "lightblue", "blue")) +
  labs(
    y = "Number of MOTUs identified",
    x = NULL,
...
plot_pca <- function(res_pca, station_labels, panel_title) {
  fviz_pca_biplot(
    res_pca,
    habillage = station_labels,
    addEllipses = TRUE,       # to visualize groups
    label = "var",            # show only variable vectors
    labelsize = 4,
    repel = TRUE,
    col.var = "black",
    col.ind = "black",
...
# Create the three PCA plots
pca_fish_plot <- plot_pca(res.pca_fish, station_labels_fish, "Fish")
pca_birds_plot <- plot_pca(res.pca_birds, station_labels_birds, "Birds")
pca_mammals_plot <- plot_pca(res.pca_mammals, station_labels_mammals, "Mammals")

# Combine the plots
combined_plot <- grid.arrange(
  pca_fish_plot,
  arrangeGrob(pca_birds_plot, pca_mammals_plot, ncol = 2),
  nrow = 2,
  heights = c(1.5, 1)
)
...
png("Fig4.png", width = 12, height = 8, units = "in", res = 1200)
par(mfrow = c(2, 3))
par(mar = c(2, 2, 2, 2))
...
ggsave("Fig6.tiff", width = 13, height = 4, units = "in", dpi = 1200, compression = "lzw")
```
